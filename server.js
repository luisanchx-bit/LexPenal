...
function initDB() {
    if (!fs.existsSync(DATA_FILE)) {
        ...
        const initialData = {
            ...
            casos: [],
            testimonios: [
                { id: 1, cliente: "Cliente anónimo", texto: "Excelente profesional", aprobado: true, fecha: "2026-01-15" }
            ],
            plantillas: [],
            tipos_caso: [
...
app.post('/api/plantillas/subir', verificarToken, verificarAdmin, upload.single('archivo'), async (req, res) => {
    try {
        ...
        if (extension === 'txt') { texto = fs.readFileSync(filePath, 'utf8'); }
        else if (extension === 'pdf') { const dataBuffer = fs.readFileSync(filePath); const pdfData = await pdfParse(dataBuffer); texto = pdfData.text; }
        else if (extension === 'docx') { const result = await mammoth.extractRawText({ path: filePath }); texto = result.value; }
...
app.listen(PORT, () => {
    ...
    console.log(`📂 Subir plantillas en /admin/plantillas.html`);
});
